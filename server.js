...
app.use('/process-data', async (req, res, next) => {
  try {
    ...
    // Process data when a certain number of entries are accumulated (e.g., every 5 entries)
    if (accumulatedData.length >= 3) {
      ...
      // Create a new Accelerometer document with processed and cleaned data
...
// Example processing function (calculate averages)
...
// Example cleaning function (remove outliers)
function removeOutliers(data) {
  // Assuming you have some logic to identify and remove outliers
  // For example, you can use statistical methods or threshold values
  // In this example, we're keeping data within the range of [1, 9] for each axis
...
// Define a route for analytics data retrieval
app.get('/analytics-data', async (req, res) => {
  try {
    // Fetch processed data from MongoDB (adjust as needed)
...
io.on("connection", (socket) => {
  ...
  // Emulate data from sensors (replace this with actual sensor data)
  setInterval(() => {
...
// Function to send data to the server for processing asynchronously
